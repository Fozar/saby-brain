...
> [!note] Ревью 2026-07-23: к `GetPartnerList` добавлено требование суммы по сделкам
> На звонке [[zvonok-musohranov-timoshenko-2026-07-23]] §1/§2 подтверждены как есть. Для `GetPartnerList` теперь решено: колонку «сумма по сделкам» считать `SUM` по полю «Сумма» `ВидЦеныДокумент` в том же подзапросе, что и `COUNT` (пока писать пусто/ноль). Детали и отложенные подзадачи — [[ReferralStub-DealSum-Field]]. Открытые вопросы по `ТипСвязи`/индексам — [[ReferralStub-Stats-Index-Questions]].
