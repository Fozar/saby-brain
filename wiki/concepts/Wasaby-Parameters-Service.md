...
## Вариант: parameters-constants

Модифицированная версия `parameters` для параметров с **периодом действия** (например, тарифные ставки, действующие с определённой даты).

Отличия:
- Дополнительное API для отбора параметров, действительных в заданный момент времени
- Нет автоматической очистки (история параметров сохраняется)
- Архитектура идентична `parameters`
