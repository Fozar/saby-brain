...
## Отчёт выполнения скрипта

Расположен в реестре **Обновления**: найти по ссылке на ВНР в строке поиска, открыть колонку «Новая версия». Карточка отчёта содержит: архив скрипта, ссылку на ВНР, результаты выполнения по каждому приложению.

Ссылку на отчёт можно скопировать в буфер («Копировать в буфер обмена») — открывается для любого пользователя с минимальными правами доступа, без дополнительной авторизации.

**Статусы по клиенту/сервису:**
...
При выполнении на онлайне скрипт запускается на клиентах, распределённых по всем приложениям; отчёт показывает разбивку успешно/ошибка/не выполнено по каждому приложению (пример из источника: online-ru — 183044 клиента обработано, 670 прервано). Файлы, созданные скриптом (`LogRedis`/`LogMsg`), скачиваются из отчёта одним архивом или по отдельности по приложениям.
...
### Файлы со списком клиентов (по стендам)

Чтобы выполнить скрипт только на конкретных клиентах, в **корень** архива добавляются файлы без расширения — по одному на стенд:

| Файл | Стенд |
|------|-------|
| `fix_cloud_clients` | fix |
| `fix-kz_cloud_clients` | fix-kz |
| `pre-test_billing_clients` | pre-test |
| `rc_cloud_clients` | rc |
| `prod_cloud_clients` | prod |
| `prod_billing_clients` | prod |
| `prod-kz_cloud_clients` | prod-kz |
| `prod-uz_billing_clients` | prod-uz |

- Клиенты указываются как ID в облаке (`_cloud_clients`) или как номер аккаунта биллинга (`_billing_clients`) — номера аккаунтов конвертируются в ID клиентов облака перед выполнением.
- Внутри файла — только числа, разделённые пробелом/табом/переносом строки.
- Если файл клиентов не указан или имеет неправильный формат — скрипт выполняется на **всех** клиентах продукта.
- Один архив со скриптом можно использовать на разных стендах: на каждом применяется свой файл клиентов. Если архив содержит только `prod_cloud_clients`, на тестовых стендах скрипт выполнится на всех клиентах, а на бою — только на указанных.
- Клиенты из списка могут принадлежать разным продуктам; при запуске на конкретном продукте выполняется только на клиентах этого продукта. Рекомендуется явно упомянуть в ВНР, что скрипт выполнится не на всех клиентах.

### Файл `services` (неперсональные приложения)

Обязателен для неперсональных приложений. В корень архива добавляется файл `services` (общий для всех стендов) с системными именами целевых сервисов, разделёнными пробелом/табом/переносом строки. Если ни один сервис из файла не найден в составе приложения — запуск завершается ошибкой.
...
### Запрос на схеме public (например, для типа hstore)

Если нужно выполнить запрос со схемой `public` (а не только схемой клиента), нельзя устанавливать `search_path` через `pgbouncer` — запрос может уйти в другое соединение. Нужно открыть **прямое** соединение к БД самостоятельно:
...
for item in db_config:
    cstring = item['master']
    db = sbis.CreateDatabase(cstring)          # прямое соединение, не через pgbouncer
    statement = db.CreateStatement(False)
    scheme = "_" + sbis.Session.ID()[:8]
    statement.Exec('SET search_path TO {}, public;'.format(scheme))
    statement.Exec(''' select 1 ''')
    db.Close()                                  # соединение создано вручную — закрывать тоже вручную
```
...
По умолчанию скрипт выполняется одновременно на каждой БД для каждого клиента, то есть максимальное число одновременно выполняющихся методов скрипта равно общему числу баз данных всех сервисов, на которых он выполняется. Хорошо подходит для SQL-скриптов в рамках схемы клиента, плохо — для скриптов с внешними (особенно асинхронными) вызовами на другие сервисы: велик риск создать на них очередь.

Ограничить можно через ВНР: поле «Ограничение одновременного выполнения» — максимальное число клиентов, одновременно выполняющих скрипт (ограничение общее по всем приложениям, по умолчанию не задано).

> [!contradiction] Диапазон значений поля
> Эта страница ранее указывала диапазон **0–100**. Источник [[skripty-2026-06-12]] (полная официальная статья «Скрипты») даёт другой диапазон: **от 0 до 999**, где `0` = без ограничений, `999` — максимально возможное значение. Более полный источник считается приоритетным; диапазон исправлен на 0–999, старое значение оставлено здесь как история расхождения.

Если скрипт помимо вызовов на сервисы создаёт сценарии DWC, нужно оценить их число и не превышать **300 000** одновременно создаваемых сценариев — иначе разделить исполнение скрипта на несколько этапов за несколько дней.

Число рабочих процессов скриптовой БЛ: по умолчанию 2 (1 для скрипта + 1 для async-вызовов на себя). Для скриптов с self-parallelism — можно запросить больше в ВНР (минимум 2). Максимум: `Управление версиямиСкриптыМаксимальноеЧислоПроцессовСкриптовыхБЛ`.
