...
**Состояние задачи:**
- Создание корешков для новых сделок: реализовано и тесты зелёные (задача 06152561)
- **Миграция корешков для существующих сделок**: реализована, но проверка не выполнена и **не может быть проведена** — зависимость от версии **400** (согласовано с **Пиреновым**)
- GUI-часть: не реализована, только консольные вызовы

**Вопрос производительности**: Мусохранов спросил о выборке всех сделок — Тимошенко пояснил, что итерация идёт по программам (не по всем сделкам), тот же подход что в реестре сделок. Принято.

**Формат сдачи (Мусохранов):** сдавать как есть + обзор по каждому пункту + декларировать планы (бой / после отпуска / когда GUI). От Мусохранова — только ревью **схемы конвертации**.

---

## 2026-06-25 — Ревью ТД итеративной загрузки (Федько, 23.06.26)

**[[soveshanie-sdacha-td-itload-2026-06-23]]**: Вторая итерация ТД по механизму порционной/итеративной загрузки реестров (`Loyalty-IterativeLoading-TD-CommonSolutions`). **Принято с замечаниями:**

1. **Диаграмма иерархии классов — бракована** (Федько не смог прочесть): нет легенды к стрелкам, нет объяснения 5 конечных классов, мелко. Переделать.
2. **Архитектурная диаграмма**: заменить метку «PriceFormation» → «BL-метод реестра»; добавить визуальный акцент что клиент **не блокируется после первой порции**.
3. **EMA-алгоритм**: упростить — формулу под спойлер, простое объяснение наверх. Добавить описание самоадаптации: EMA-коэффициент в `GlobalParams` уточняется со временем — чем больше пользуются реестром, тем точнее.
4. **Параметры per method**: явно объяснить в ТД, почему ключ привязан к методу, а не к реестру.

**Новая сущность**: [[Ютман-Элина]] (ответственная за бонусы, финальный согласующий ТД).

Согласование финального варианта — [[Ютман-Элина]], не Федько.
