...
Last updated: 2026-08-25 | Total pages: 456 | Sources ingested: 233
...
- [[ReferralProgram-History-UI-Design]] (c-000229) — UI истории изменений по офферам/лидам/корешкам: раздельные кнопки истории для оффера и для корешка, объединение в одну историю явно отвергнуто; разбор реализации 07-29 — Лебедева кнопка корешка, Тимошенко клик-колбэк + фильтрация GetHistoryList (status: developing)
- [[referral-history-implementation-breakdown-2026-07-29]] (c-000230) — разбор реализации UI истории реф. программы: 3 пункта (кнопка корешка / клик-колбэк / фильтр GetHistoryList), распределение между Тимошенко и Лебедевой (status: current)
...
- [[RecruitmentReferral-BL-Scope-Estimate]] (c-000279) — объём и оценка БЛ (~22 д против 6 строк «???д» в плане): новый `ProgramType.RECRUITMENT`, резолвер «вакансия → программа», корешок кандидата поверх `CreateStub`/`UpdateStub`; противоречие «реф-код = источник партнёра или участника» (status: developing)
...
- [[ReferralStub-LeadSum-Implementation]] (c-000259) — задача №08052776: сумма сделки пишется в колонку `Сумма` `ВидЦеныДокумент` через поле API `LeadSum` (`CreateStub`/`UpdateStub`/`CreateStubsForExistingLeads`/`HandleLeadStateChanged`), отдаётся в `ReadStub`/`GetStubList`; в отличие от `Price` обновляется при любом статусе и без OWNER_ADMIN. `ДокументРасширение.Сумма` входит в стандартный формат `GetRawDocs` — доказано через `Set` vs `Put` в `get_lead_list.py`. Доброска существующим корешкам не сделана (status: active)
- [[ReferralProgram-GetLeadPeriodList-Stub-Mode]] (c-000254) — задача №07294792: мастер-фильтр реестра «Заявки» (`GetLeadPeriodList`) переведён на корешки под `ref_deals_convert`. `LeadCount` по корешкам был и так; чинился `Price` — корешок сделки после миграции не имеет `"Документ"` и не попадал ни в одну ветку `IsStubMode = "0"`. Добавлен отдельный признак `IsSabyBank`, чтобы у обычных программ не пропали вознаграждения за посетителей. Маркетинг здесь неприменим: `ReadStat` не даёт разбивки по месяцам, а фильтр тянет до 40 периодов за страницу (status: active)
...
- [[DiscountCardType-Create-Design-Constructor]] (c-000278) — задача №06242691: диалог создания типа карты на новом конструкторе Дизайна ДК. Задача передана Тимошенко, рабочий предмет — **общий код на три пересекающиеся задачи**, всё частное за границей. ТР переписано трижды: сайт на бэке не создаём, блоки обратной стороны заполняет UI, реф. ссылка удалена. `DiscountCardType.Init` расширить нельзя — он сидит в CRUD-биндинге фронта как `update`, поэтому Ютман согласовала **отдельный метод**; реализован `DiscountCardTemplate.GetBackSideData` (+ `.uax`, тесты). Осталась инициализация данных ПО ДизайнКарты: ПО объявлено в `DiscountCard.aorx` с пустыми `<methods/>`, логотипа среди свойств нет, формат записи неизвестен — ждём реализацию ПО (Кузаков). Плюс засада: `CardTypeTemplate.Update` пересобирает `ViewDetails` из `ThemeData` брендбука (status: developing)
...
- [[Bonus-GetTotalBalance-Local-Card-Scan-Memory]] (c-000207) — задача 07208958: `GetBonusesNew` ест ~1 ГБ/итерация на `test-inside`; отложенный хвост 05292113 — после починки СДК-ветки (`1be452e6e2`) доминирующим стал локальный скан `Карта`. **«1 ГБ» = 136 348 буферов**, замерено напрямую на стенде. Джойн к `ЧастноеЛицо` (71%) нужен только ради `IsFranchise` → условный шаблон даёт 136 348 → 38 938 (3.5x), тесты 10/10. **Открытый вопрос закрыт: аккаунт франшизный**, значит шаг 1 этот тикет не закрывает. Ключевая методическая находка: `!has_franchise` уходит **связанным параметром**, при литерале PostgreSQL сам удаляет джойн и замер «до» становится фиктивным. Шаг 2 — частичный индекс `BonusCardEmission` по `Карта` (**таблица наша**, `PriceFormation.Online.dicx:1773` — прежний тезис про `dccommon`/СДК опровергнут), валидирован локально 13 998 → 1 098. Альтернатива без индекса — фильтр по флагу типа карты, 29 781 → 1 802, но −0.34% баланса (status: developing)
...
- [[zvonok-musohranov-timoshenko-2026-07-23]] (c-000210) — 2026-07-23 | ревью подсчёта статистики по корешкам: `GetPartnerList` строго по реф. программе; решение хранить сумму по сделкам в поле «Сумма» ВЦД (`SUM` рядом с `COUNT`, пока пусто); открытые вопросы по `ТипСвязи`/индексам
...
- [[Loyalty-Events-External-Consumers]] (c-000273) — у удаляемых событий лояльности есть подписчики вне СДК (SabyGet, SabyForms); паттерн re-publish своего события из discount-cards, поле `application` (status: developing)
- [[DWC-Card-Events-Migration]] — миграция Card-событий на DWC; забытые точки `async_notify_changed_cards` и `BonusOperationEventQueue.publish` добраны в №08106197 (status: developing)
...
- [[Card-IsBonus-Flag]] (c-000255) — два денормализованных флага с именем `IsBonus`: у карты (`_calc_is_bonus` = были операции ИЛИ действует программа ИЛИ франшизные операции) и у типа карты (только действующие программы). Не эквивалентны: на стенде 199 карт бонусные по карте, но не по типу — 0.34% баланса. `GetTotalBalance` и `GetBonusesNew` питают один виджет, но фильтруют эмиссионные карты по **разным** флагам. В коде TODO о миграции к флагу карты; флаг лежит в JSONB, отсюда нужда в частичном индексе (status: current)
...
- [[DiscountCard-Design-Constructor-WorkPlan]] — 252,5 чд по 5 этапам + 89,5 чд управления; срок 20.02.2027; команда из 31 участника и концептуальный план тестирования (обновлено 2026-08-06) (status: developing)
- [[discount-card-design-constructor-attachments-2026-08-06]] (c-000256) — догоняющий ингест вложений карточки проекта: команда/роли из плановой сметы, план тестирования (Куимова Н. В., выпуск под фичей 26.6100), сверка «план vs смета»; «Сравнение с конкурентами» и «Эксплуатация системы» — пустые шаблоны. Метод: `sbis_resolve_url` → `doc_id` → `sbis_list_attachments` (status: current)
...
- [[ReferralStub-DealSum-Field]] — сумма по сделкам в корешках: хранить в поле «Сумма» ВЦД, `SUM` рядом с `COUNT` в подзапросе; реализовано 2026-08-06 как поле `LeadSum` — [[ReferralStub-LeadSum-Implementation]] (status: resolved)
...
- [[DWC-Card-Events-Migration]] — реализация этапа «Карты→DWC»: feature flag `dwc_card`; №08106197 добирает забытые точки, QA-сценарии на странице (status: developing)
- [[Loyalty-Events-External-Consumers]] (c-000273) — №03266097: удаление 14 событий; SabyGet и SabyForms надо переводить на события СДК, тексты задач и ответственные (status: developing)
...
- [[Черных-Константин-Евгеньевич]] (c-000257) — менеджер проекта «Перевод дизайна дисконтных карт на конструктор»; владелец карточки проекта, автор сверки «план vs смета» (status: current)
- [[Куимова-Наталья-Владимировна]] (c-000258) — инженер-тестировщик 1, ответственный функциональный тестировщик проекта «Дизайн ДК на конструкторе»; держит концептуальный план тестирования (status: current)
