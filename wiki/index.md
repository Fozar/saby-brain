...
- [[DiscountCardType-Create-Design-Constructor]] (c-000278) — задача №06242691: диалог создания типа карты на новом конструкторе Дизайна ДК. К 25.08 задача передана Тимошенко (Егор снял Михайленко) и требует **общего кода на три пересекающиеся задачи**. ТР переписано трижды: сайт на бэке не создаём, ПО к сайту не привязываем, блоки обратной стороны заполняет UI, реф. ссылка удалена; `Init` сменил смысл — теперь возвращает `{address, phone}` по ТП. `Create` инициализирует `ViewDetails` баннером/логотипом/названием с **опубликованной** ТП и отдаёт `CardTemplateId`. Засада: `CardTypeTemplate.Update` пересобирает `ViewDetails` из `ThemeData` брендбука. `SiteId` уже в контракте СДК на `rc-26.5100`. Реализована часть по ТП; ветка от 13.08 частично устарела (status: developing)
