...
- From: conversation on задачу №06242691 — разбор переписки, трёх редакций ТР, иерархии проекта и кода СДК на rc-26.5100
- Key insight: Егор снял с задач Михайленко и передал их Тимошенко, требуется общий код на три пересекающиеся задачи. ТР от 25.08 срезало БЛ до создания записи `CardTemplate` с инициализацией `ViewDetails` (баннер, логотип, название — с **опубликованной** ТП) и нового метода `Init`, который теперь просто возвращает `{address, phone}`: блоки обратной стороны заполняет UI, реф. ссылка из ТР удалена, сайт на бэке не создаётся, пункт про чистку черновиков отпал. Главная засада реализации — `CardTypeTemplate.Update` в СДК безусловно пересобирает `ViewDetails` из `ThemeData` брендбука (`update.py:88`), поэтому при новом конструкторе `ThemeData` слать нельзя.
- Tooling: свежая редакция `.sabydoc` не приходит по href из вложения задачи (`.../api/v1/<id>_<version>` — зафиксированный снимок); актуальная версия отдаётся тем же адресом **без второго uuid**
- Updated: [[DiscountCardType-Create-Design-Constructor]] (status blocked → developing, помечены устаревшие части зашелвленной реализации от 13.08)

## [2026-08-14] save | Реферальная система в подборе персонала — объём и оценка БЛ

- Type: synthesis
- Location: wiki/questions/RecruitmentReferral-BL-Scope-Estimate.md
- From: conversation on поручение №011388221035800526 — «написать тех. реализацию и заполнить план работ», разбор ТЗ (04.08), плана работ (13.08) и концепта проекта
- Key insight: план работ содержит 6 строк БЛ «Тимошенко А. ???д» в двух этапах, тогда как бэк лояльности участвует в 9 этапах из 12 — реальный объём ~22 д против 6 д, заложенных в §6.3.2 ТЗ. Новой сущности не появляется: корешок кандидата = корешок SabyBank РКО (`CreateStub`/`UpdateStub`/`GetStubList` переиспользуются), меняется только тип целевого документа. Два блокера: (1) «реф-код это источник партнёра, а не участника» из плана противоречит §6.4 ТЗ и коду (карта на пару «партнёр × программа») — при буквальном чтении это переделка модели +3…5 д и миграция; (2) `UpdateStub`/`SetStubPrice` требуют роли `OWNER_ADMIN`, под которой служебный триггер подбора не исполняется — нужен служебный путь фиксации начисления.
- Tooling: `convert_sabydoc_to_markdown` обрезает абзацы на форматированных фрагментах — так пропали все красные комментарии Мусохранова и все строки БЛ; `.sabydoc` — zip, полный текст восстанавливается парсингом `index.site` (узлы `ModuleEditor/formattedText:View`)
- Updated: [[RecruitmentReferral-TZ]] (редакция 04.08, схема хранения §6.4, расхождения с кодом), [[RecruitmentReferral-WorkPlan]] (редакция 13.08, ответственные, оценённые строки, закрытые развилки)
