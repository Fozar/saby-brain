...
# nomic-embed-text context limit: Cyrillic text uses ~1 token/byte (byte-level BPE),
# so 8192-token context ≈ 4096 UTF-8 bytes for dense Cyrillic. Use 4000 to be safe.
    ...
    def _lock_cache():
        ...
        while True:
            try:
                ...
                # Stale lock? Check if owning PID is still alive.
                try:
                    pid = int(CACHE_LOCK.read_text().strip())
                    os.kill(pid, 0)  # raises OSError if process is gone
                except (OSError, ValueError):
                    CACHE_LOCK.unlink(missing_ok=True)
                    continue
                if time.monotonic() >= deadline:
                    raise OSError(f"could not acquire tiling cache lock within {_LOCK_TIMEOUT}s")
                time.sleep(0.05)

    def _unlock_cache(fd) -> None:
        CACHE_LOCK.unlink(missing_ok=True)
...
def _truncate_for_embed(text: str) -> str:
    ...
    if len(encoded) <= MAX_EMBED_BYTES:
        return text
    # Slice bytes, then decode safely (drop the last partial multi-byte char).
    cut = encoded[:MAX_EMBED_BYTES].decode("utf-8", errors="ignore")
    # Trim to last whitespace to avoid splitting a word mid-token.
    last_ws = cut.rfind(" ")
    return cut[:last_ws] if last_ws > MAX_EMBED_BYTES // 4 else cut


def embed(text: str, model: str, url: str) -> list[float]:
    data = _http_post_json(
        f"{url}/api/embeddings",
        {"model": model, "prompt": _truncate_for_embed(text)},
        EMBED_TIMEOUT_SEC,
    )
